...
void TypeLetter(char L){
	...
	while (cur->l != cur->r){
		...
		int mid = (cur->l + cur->r) >> 1;
		if (pos <= mid){
			t[tot].l = cur->l, t[tot].r = mid,
			cur->lson = &t[tot], cur->rson = old->rson, old = old->lson;
		}else{
			t[tot].l = mid + 1, t[tot].r = cur->r,
			cur->lson = old->lson, cur->rson = &t[tot], old = old->rson;
		}
		cur = &t[tot++];
	}
	// cerr << "cur = "<< (cur-t) << ' ' << cur->l << ' ' << cur->r << ' ' << '\n';
...
char GetLetter(int P){
	...
	while (cur->l != cur->r)
		cur = (P <= ((cur->l + cur->r) >> 1)) ? (cur->lson) : (cur->rson);
	return cur->val;
}