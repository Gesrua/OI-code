...
void build(node* cur) {
    if (cur->l == cur->r) {
        cur->fa = cur->l, cur->dep = 0;
        return;
    }
    int mid = (cur->l + cur->r) >> 1;
    t[cnt].l = cur->l, t[cnt].r = mid, cur->lson = &t[cnt];
    ++cnt;
    t[cnt].l = mid + 1, t[cnt].r = cur->r, cur->rson = &t[cnt];
    ++cnt;
    build(cur->lson);
    build(cur->rson);
}
extern inline node* query(node* cur, int s) {
    int mid;
    while (cur->l != cur->r) mid = (cur->l + cur->r) >> 1, cur = (s <= mid) ? (cur->lson) : (cur->rson);
    return cur;
}
node* modify(node* cur) {
    node* rtn = &t[cnt++];
    rtn->l = cur->l, rtn->r = cur->r, rtn->fa = cur->fa;
    ...
    int mid = (cur->l + cur->r) >> 1;
    if (goal.l <= mid)
        rtn->lson = modify(cur->lson), rtn->rson = cur->rson;
    else
        rtn->lson = cur->lson, rtn->rson = modify(cur->rson);
    return rtn;
}
...
int main() {
    ...
    for (int i = 1; i <= m; ++i) {
        ...
        if (opt == 1)
            ...
            his[i] = his[i - 1];
            if (find(his[i], query(his[i], rd())) == find(his[i], query(his[i], rd())))
                puts("1");
            else
                puts("0");
        }
