...
long long caculate(struct node a, long long t) { return a.a - (t + a.c) * a.b; }
long long T, n;
...
void solve(long long s, long long t) {
    ...
    /*
       if (s == n && t+data[s].c <= T) {dp[s][t] = max(caculate(data[s],t),0);
       return;} if (s == n) {dp[s][t] = 0;return;}
     */
    if (s == n) {
        if (t + data[s].c <= T) {
            dp[s][t] = max(caculate(data[s], t), 0);
            return;
        } else {
            dp[s][t] = 0;
            return;
    ...
    if (t + data[s].c <= T)
        ...
        dp[s][t] = max(dp[s + 1][t], dp[s + 1][t + data[s].c]);
}
int main() {
    long long i;
