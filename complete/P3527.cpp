...
bool chk(int x){
    ...
    for (std::vector< int >::iterator i = pos[id].begin(); i != pos[id].end(); ++i){
        sum += seg.ask(*i, *i);
        if (sum >= goal) {/*cerr << "pass" << endl;*/ return 1;}
    }
    // cerr << "fail" << endl;
    c[x].tar -= sum;
    return 0;
...
void solve(int l, int r, int x, int y){
    ...
    if (l > r || x > y) return;
    rep(i, l, mid){
        if (star[i].l <= star[i].r) seg.add(star[i].l, star[i].r, star[i].w);
        else seg.add(star[i].l, n, star[i].w), seg.add(1, star[i].r, star[i].w);
    }
    int cnt1 = 0, cnt2 = 0;
    rep(i, x, y){
        if (chk(i)) t1[cnt1++] = c[i];
        else t2[cnt2++] = c[i];
    }
...
bool cmp_id(const n2& a, const n2& b){
    return a.id < b.id;
}

// int t;
// int read(){int x=0;while(t=getchar(),!isdigit(t));while(x=(x<<1)+(x<<3)+t-48,t=getchar(),isdigit(t)); return x;}
...
int main(){
    ...
    rep(i, 1, m) { cin >> c[i].tar; c[i].id = i, c[i].ans = 0; }
    cin >> k;
    ...
    rep(i, 1, m){
        if (c[i].ans == 0 || c[i].ans == k) cout << "NIE" << endl;
        else cout << c[i].ans << endl;
