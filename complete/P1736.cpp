...
inline int min(int a, int b, int c) { return std::min(std::min(a, b), c); }
int main() {
    ...
    for (i = 1; i <= n; ++i) {
        for (j = 1; j <= m; ++j) {
            if (a[i][j]) {
                dp[i][j] = max(min(dp[i - 1][j - 1] + 1, row[i][j - 1] + 1, col[i - 1][j] + 1), min(dp[i - 1][j + 1] + 1, col[i - 1][j] + 1, rowr[i][j + 1] + 1));
