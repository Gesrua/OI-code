...
struct union_find_set {
    ...
    inline void unite(const int x, const int y) { fa[find(x)] = find(y); }
    inline bool chk(const int x, const int y) { return find(x) == find(y); }
} s;
int main() {
    ...
    std::sort(e, e + m, [](const edge& a, const edge& b) { return a.w < b.w; });
