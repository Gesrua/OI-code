...
struct node{
    ...
    bool operator< (const node b){
        ...
        if(this->b < b.b) return true;else return false;
    }
    node(node &x, int type){
        switch(type){
            case 1:*this = node(x.cur+1,x.a-1,x.b,x.c,x.d);break;
            case 2:*this = node(x.cur+2,x.a,x.b-1,x.c,x.d);break;
            case 3:*this = node(x.cur+3,x.a,x.b,x.c-1,x.d);break;
            case 4:*this = node(x.cur+4,x.a,x.b,x.c,x.d-1);break;
        }
    }
...
int v[31], n, m;
inline int max_yuan(int a, int b){
    return a>b?a:b;
}
inline int max(int a, int b, int c, int d){
    return max_yuan(max_yuan(a,b), max_yuan(c,d));
}
...
//     ans = max(solve(cur+1,a-1,b,c,d),solve(cur+2,a,b-1,c,d),solve(cur+3,a,b,c-1,d),solve(cur+4,a,b,c,d-1));
//     if (ans == -2) return -2;
