...
struct p {
    int x, y;
    p() { x = 0, y = 0; }
    bool operator<(const p b) { return x != b.x ? x < b.x : y < b.y; }
} a[1010];
struct node {
    bool a[300][300];
    ...
    inline void upd(const int x, const int y, const bool opt) { this->a[x + 100][y + 100] = opt; }
    node() { std::memset(a, 0, sizeof(a)); }
} map;
