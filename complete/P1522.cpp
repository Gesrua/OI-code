...
using std::isdigit; using std::cin; using std::cout; using std::endl; using std::sqrt;
struct point {
	int x, y;
	point () : x(0), y(0){
	}
...
inline void unity(const int x, const int y){
	fa[x] = find(fa[y]);
}
...
int main(){
	...
	for (int i = 1; i <= n; ++i)
		for (int j = 1; j <= n; ++j)
			if (find(i) != find(j))
				ans = min(dis[i] + dis[j] + g_dis[i][j], ans);
