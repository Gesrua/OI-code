...
struct ChairmanTree {
    struct Node {
        ...
        Node* rs;
        int cnt, l, r;
        Node(int _l, int _r, Node* _ls = NULL, Node* _rs = NULL) { l = _l, r = _r, ls = _ls, rs = _rs, cnt = (_ls ? _ls->cnt : 0) + (_rs ? _rs->cnt : 0); }
        Node(int _l, int _r, int _cnt) { l = _l, r = _r, cnt = _cnt, ls = NULL, rs = NULL; }
        void pushdown() {
            int mid = (l + r) / 2;
        ...
        Node* insert(int x) {
            if (x < l || r < x)
                ...
                // if (x <= mid) return new (pool.alloc(sizeof(Node))) Node(l,
                // r, ls->insert(x), rs); else return new
                // (pool.alloc(sizeof(Node))) Node(l, r, ls, rs->insert(x));
            }
        }
        int rank() { return ls ? ls->cnt : 0; }
        void print() {
            cerr << l << ' ' << r << ' ' << cnt << endl;
            if (ls) ls->print();
            if (rs) rs->print();
    ...
    int query(int _l, int _r, int k) {
        ...
        while (min != max) {
            ...
            if (k <= rk)
                l = l->ls, r = r->ls, max = mid;
            else
                l = l->rs, r = r->rs, k -= rk, min = mid + 1;
        }
        // cerr << "******" << endl;
        return min;
    }
