...
int main(){
    ...
    while (cin >> l >> r){
        ...
        for (int i = 1; i <= tot && r >= prime[i]; ++i){
            unsigned int j;
            if (l <= prime[i]) j = prime[i]*2;
            else if (l % prime[i] == 0) j = l;
        ...
        if (out) cout << "There are no adjacent primes.\n";
        else cout << minl+l << ',' << minr+l << " are closest, " << maxl+l << ',' << maxr+l << " are most distant.\n";
    }
    return 0;
}