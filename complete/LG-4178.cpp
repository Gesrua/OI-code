...
struct graph {
    ...
    int tot;
    graph() { tot = 0; }
    inline void _addedge(int u, int v, int w) { e[tot].v = v, e[tot].w = w, e[tot].nxt = p[u].first, p[u].first = &e[tot++]; }
...
struct array {
    int a[N], tot;
    array() { tot = 0; }
    void push_back(int x) { a[tot++] = x; }
    void pop_back(int x) { --tot; }
    int& operator[](int x) { return a[x]; }
    int* begin() { return a; }
    int* end() { return a + tot; }
    void clear() { tot = 0; }
    int size() { return tot; }
} s;
